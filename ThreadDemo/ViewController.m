...
static NSString *kURL =
    @"http://127.0.0.1:8020/GangFuTong/img/checkfocus1@2x.png";
  ...
  @synchronized(nil) {
    while (TRUE) {
      if (tickets > 0) {
        ...
        count += 1;
        NSString *threadName = [[NSThread currentThread] name];
        NSLog(@"执行了%ld次，还剩下%ld张票,线程名：%@",
              (long)count, (long)tickets, threadName);
      } else {
        break;
      }
    }
...
- (void)operationTest {
  NSInvocationOperation *invocationOption =
...
- (void)testBarrierAsync {
  dispatch_queue_t queue =
      dispatch_queue_create("gcdtest.rongfzh.yc", DISPATCH_QUEUE_CONCURRENT);
